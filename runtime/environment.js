 ...
 * API is used, js-dom-literal only handles the construction of new objects via XML literal syntax.
 *
...
~function() {
  ...
  XMLEnvironment.prototype = {
    ...
    _frag: function(descriptor) {
      return this.createFragment(this._generateChildren(descriptor, this._namespaces));
    },
    ...
    _generateChildren: function(descriptors, namespaces) {
      ...
      for (var ii = 0; ii < descriptors.length; ++ii) {
        ...
        if (descriptor.type === 3) { // TYPE_EXPR
          if (this.isNode(descriptor.value)) {
