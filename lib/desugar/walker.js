...
var t = require('../../external/narcissus-xml').definitions.tokenIds;
		...
		switch (type) {
			...
			case t.XML_LITERAL:
				if (visitors[type]) {
					...
					node.xmlAttributes.map(walk);
					node.xmlChildren && node.xmlChildren.map(walk);
				}
				break;

			case t.XML_ATTRIBUTE:
				if (visitors[type]) {
					visitors[type].call(node);
				} else {
					if (node.xmlValue.type) {
						walk(node.xmlValue);
					}
				}
