...
~function() {
		...
		_generateChildren: function(descriptors, namespaces) {
			...
			for (var ii = 0; ii < descriptors.length; ++ii) {
				...
				if (descriptor._type === 1) { // TYPE_ELEMENT
					// Check tag match
					var open = descriptorToString(descriptor._open);
					var close = descriptorToString(descriptor._close);
					...
					var keys = descriptor._attributes && descriptor._attributes._keys;
					var vals = descriptor._attributes && descriptor._attributes._vals;
					...
					// Generate children
					var children = descriptor._content ?
						this._generateChildren(descriptor._content, currentNamespaces) : undefined;
				...
				if (descriptor._type === 3) { // TYPE_EXPR
					if (descriptor._value === undefined || descriptor._value === null) {
						continue;
					} else if (this.isNode(descriptor._value)) {
						elements.push(descriptor._value);
						continue;
					} else {
						descriptor._value = ToString(descriptor._value);
					}
				} else if (descriptor._type === 2) { // TYPE_CDATA
					if (this.ignoreWhitespace) {
						...
						descriptor._value = descriptor._value.replace(newlineWithSpaces, '\n');

						// Then remove newlines from the front and back
						descriptor._value = descriptor._value.replace(leadingAndTrailingNewlinesRegex, '');
					}
				}
				if (descriptor._value !== '') {
					var element = this.createTextNode(descriptor._value);
					if (element) {
						elements.push(element);
					}
				}
	...
	function descriptorToString(descriptor) {
		...
		return descriptor._type === 3 ? ToString(descriptor._value) : descriptor;
	}
}.call(this);
